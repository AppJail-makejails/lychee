...
if [ "$RUN_AS_ROOT" = "yes" ]; then
  ...
  echo "✅ Will drop privileges to noroot user"
fi

# Helper function to run commands as noroot
run_as_noroot() {
...
web)
  ...
  run_as_noroot php artisan migrate --force

  # Clear and cache configuration
  echo "🧹 Optimizing application..."
  run_as_noroot php artisan config:clear
  run_as_noroot php artisan config:cache
  run_as_noroot php artisan route:clear
  run_as_noroot php artisan route:cache
  run_as_noroot php artisan view:clear
  run_as_noroot php artisan view:cache

  echo "✅ Application ready!"

  if [ ! -f "/app/frankenphp_target" ]; then
    # Start PHP-FPM and Nginx for traditional setup
    echo "🚀 Starting PHP-FPM..."
    php-fpm -D -y /usr/local/etc/php-fpm.conf -g /var/run/php-fpm.pid
  fi

  # nginx will drop privileges by itself.
  if [ "$RUN_AS_ROOT" = "yes" ] || [ "$1" = "nginx" ]; then
    if [ "$1" = "nginx" ]; then
      echo "🚀 Starting NGINX..."
    fi
    exec "$@"
...
worker)
  ...
  while [ "$migration_attempt" -lt "$max_migration_attempts" ]; do
    ...
    if run_as_noroot php artisan migrate:status 2>/dev/null | grep -q "Pending"; then
  ...
  while $KEEP_RUNNING; do
    ...
    run_as_noroot php artisan queue:work \
      --queue="$QUEUE_NAMES" \
      --tries=3 \
      --timeout=3600 \
      --sleep=3 \
